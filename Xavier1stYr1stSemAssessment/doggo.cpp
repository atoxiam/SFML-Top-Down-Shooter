...
sf::Texture doggo::m_enemyTex;
sf::Texture doggo::m_healthTex;
...
doggo::doggo(){
	...
	m_enemyTex.loadFromFile("graphics/enemies/doggo.png");
...
void doggo::Update(sf::RenderWindow &window, float elapsedTime){
...
void doggo::Render(sf::RenderWindow &window){
	if (m_active)
	...
	if (sprite.getGlobalBounds().intersects(sf::Rect<float>((float)sf::Mouse::getPosition(window).x, (float)sf::Mouse::getPosition(window).y + 1.0f, 1.0f, 1.0f))){
		if (!m_hasTargetTexture){
			m_enemyTex.loadFromFile("graphics/enemies/doggo_target.png");
			sprite.setTexture(m_enemyTex);
...
void doggo::SetPosition(float x, float y){
...
void doggo::reduceHealth(int pDamage){
	m_health -= pDamage;
}


//Healthbar init
void doggo::initHealthBar(){
...
void doggo::setActiveBool(bool active){
	m_active = active;
}