...
Boss3::Boss3(){
	...
	m_shootcamels = false; //shoots or not
	...
	camelSprite.setPosition(sprite.getPosition().x + 100, sprite.getPosition().y + 150);
...
void Boss3::Update(sf::RenderWindow &window, float elapsedTime){
	if (m_active) {
		...
		m_cmy = camelSprite.getPosition().y;
...
void Boss3::Render(sf::RenderWindow &window)
{
	if (m_active){
		if (m_camelMActive)
			window.draw(camelSprite); //machine
...
void Boss3::stateInit0(){
	...
	camelSprite.setPosition(m_xPos + 100, m_yPos + 150);
...
void Boss3::stateInit2(){
	if (m_cmy <= m_yPos + 200){
		...
		camelSprite.setPosition(m_xPos + 100, m_yPos + 200);
	}
